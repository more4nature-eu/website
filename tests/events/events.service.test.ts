...
describe('EventsService', () => {
  ...
  const fixedDate = new Date('2024-08-21T00:00:00Z');
    ...
    expect(pastEvents).toEqual([
      {
        ...
        description:
          '<p>In May, more4nature partners gathered in Copenhagen to discuss and advance the projects&apos; ongoing work to transform conservation efforts using Citizen Science initiatives in the process of environmental compliance assurance.</p><p>Over three days, project partners shared updates, discussed cases and ideas for moving promoting Citizen Science initiatives in environmental compliance assurance. The partners also met with the European Environmental Agency to discuss the agency’s role in aggregating data and information from EU member states for compliance reporting and finding areas for collaboration.</p><p>Finally, attendees took a field trip to Jægersborg Deer Park to learn from a local forest ranger in charge from the Nature Agency, about the actions taken to enhance biodiversity, one of the key more4nature themes.</p>',
        date: '2024-05-28',
  ...
  it('should return upcoming events filtered by name', () => {
    const filters: Partial<Event> = { title: 'Event 2' };
    eventsService = new EventsService(EVENTS, filters);
    const upcomingEvents = eventsService.getUpcomingEvents();
  ...
  it('should return past events filtered by name', () => {
    const filters: Partial<Event> = { title: 'Seagrass and Pollution' };
    eventsService = new EventsService(EVENTS, filters);
    const pastEvents = eventsService.getPastEvents();
  ...
  it('should return upcoming events filtered by location and name', () => {
    const filters: Partial<Event> = { location: 'New York', title: 'Event 1' };
    eventsService = new EventsService(EVENTS, filters);
    const upcomingEvents = eventsService.getUpcomingEvents();
